...
public abstract class SMSDispatcher extends Handler {
    ...
    protected void sendRawPdu(SmsTracker tracker) {
        ...
        if (!mUsageMonitor.check(tracker.mAppPackage, SINGLE_PART_SMS)) {
        ...
        public final void onReceive(Context context, Intent intent) {
            if (getResultCode() != Activity.RESULT_CANCELED) {
                ...
                try {
                    ...
                    int subscription = intent.getIntExtra(SUBSCRIPTION_KEY, -1);
                    ...
                    if (messages != null && messages.length > 0) {
                        ...
                        Log.d(TAG,
                                String.format(
                                        "Mock SMS. scAddress: %s, senderAddress: %s, msg: %s, sub: %d",
                                        scAddress, senderAddress, msg, subscription));
                    ...
                    if (MSimTelephonyManager.getDefault().isMultiSimEnabled()) {
                        if (subscription < 0 || subscription >= numPhones) {
                            mockSmsIntent.putExtra(SUBSCRIPTION_KEY,
                                    MSimSmsManager.getDefault().getPreferredSmsSubscription());
                        } else {
                            mockSmsIntent.putExtra(SUBSCRIPTION_KEY, subscription);
                        }
                    }
